...
export default function AdminPage() {
  ...
  if (!loggedIn) {
    return (
      <div className="min-h-screen flex items-center justify-center px-5 bg-n-50">
        <div className="bg-white rounded-2xl p-10 max-w-sm w-full border border-n-100 text-center">
          <a href="/" className="inline-block mb-6">
            <Image src="/images/logo.png" alt="공간토리" width={200} height={82} className="h-14 w-auto mx-auto" />
          </a>
          <h1 className="text-lg font-bold text-n-900 mb-1">관리자 로그인</h1>
          <p className="text-sm text-n-400 mb-6">비밀번호를 입력해주세요</p>
          ...
          <input type="password" value={pw} onChange={e => { setPw(e.target.value); setPwError(false); }} onKeyDown={e => e.key === "Enter" && tryLogin()}
            className="w-full px-4 py-3 rounded-xl border border-n-200 text-sm mb-4 focus:outline-none focus:border-n-400" placeholder="비밀번호" autoFocus />
          <button onClick={tryLogin} className="w-full py-3 rounded-xl bg-n-900 text-white text-sm font-medium hover:bg-n-800 transition-colors">로그인</button>
        </div>
      </div>
    );
  }
  ...
  const inputClass = "w-full px-3 py-2.5 rounded-lg border border-n-200 text-sm focus:outline-none focus:border-n-400";

  return (
    <div className="min-h-screen bg-n-50">
      {/* Header */}
      <div className="sticky top-0 z-50 bg-white border-b border-n-100">
        <div className="mx-auto max-w-5xl flex items-center justify-between px-6 h-14">
          <div className="flex items-center gap-3">
            <a href="/">
              <Image src="/images/logo.png" alt="공간토리" width={100} height={34} className="h-7 w-auto" />
            </a>
            <span className="text-xs text-n-400 font-medium">관리자</span>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={logout} className="text-xs px-3 py-1.5 rounded-lg border border-n-200 text-n-500 hover:bg-n-100 transition-colors">로그아웃</button>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div className="bg-white border-b border-n-100">
        <div className="mx-auto max-w-5xl flex px-6">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`px-5 py-3 text-sm font-medium border-b-2 transition-colors ${tab === t.key ? "text-n-900 border-n-900" : "text-n-400 border-transparent hover:text-n-600"}`}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-5xl px-6 py-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div key={i} className="bg-white rounded-xl p-4 border border-n-100">
              <p className="text-xl font-bold text-n-900">{s.num}</p>
              <p className="text-xs text-n-400">{s.label}</p>
            </div>
          ))}
        </div>
        ...
        {/* Payment form */}
        {tab === "payments" && (
          <div className="bg-white rounded-xl p-6 border border-n-100 mb-6">
            <h3 className="text-sm font-semibold text-n-800 mb-4">새 결제 링크 생성</h3>
            ...
            <button onClick={createPayment} className="px-5 py-2.5 rounded-lg bg-n-900 text-white text-sm font-medium hover:bg-n-800 transition-colors">결제 링크 생성</button>
          </div>
        )}
        ...
        <div className="space-y-3">
          {tab === "payments" ? (
            payments.length === 0 ? <p className="text-center text-n-400 py-12">생성된 결제 내역이 없습니다</p> :
            payments.sort((a, b) => b.id - a.id).map(p => (
              <div key={p.id} className="bg-white rounded-xl p-5 border border-n-100">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 font-medium">결제</span>
                    <span className="font-semibold text-n-800">{esc(p.name)}</span>
                  ...
                  <span className="text-xs text-n-400">{esc(p.date)}</span>
                </div>
                <div className="flex flex-wrap gap-4 text-xs text-n-500 mb-2">
                  <span>연락처: <strong className="text-n-700">{esc(p.phone)}</strong></span>
                  <span>서비스: <strong className="text-n-700">{esc(p.service)}</strong></span>
                  <span className="text-lg font-bold text-primary-600">{Number(p.amount).toLocaleString()}원</span>
                </div>
                {p.memo && <p className="text-xs text-n-500 bg-n-50 p-3 rounded-lg mb-2">{esc(p.memo)}</p>}
                <div className="flex gap-2 mt-3">
                  {p.status === "pending" && <>
                    ...
                    <button onClick={() => showPaymentLink(p.id)} className="text-xs px-3 py-1.5 rounded-lg border border-n-300 text-n-600 hover:bg-n-50">링크 보기</button>
            ...
            (() => {
              ...
              if (items.length === 0) return <p className="text-center text-n-400 py-12">아직 신청 내역이 없습니다</p>;

              return items.map(item => (
                <div key={item.id} className="bg-white rounded-xl p-5 border border-n-100">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-n-800">{esc(item.name)}</span>
                    </div>
                    <span className="text-xs text-n-400">{esc(item.date)}</span>
                  </div>
                  <div className="flex flex-wrap gap-4 text-xs text-n-500">
                    <span>연락처: <strong className="text-n-700">{esc(item.phone)}</strong></span>
                    <span>지역: <strong className="text-n-700">{esc(item.address) || "-"}</strong></span>
                    {item._type === "survey" && <span>서비스: <strong className="text-n-700">{esc((item as Survey).service) || "-"}</strong></span>}
                  </div>
                  {item._type === "contact" && (item as Contact).message && (
                    <p className="text-xs text-n-500 bg-n-50 p-3 rounded-lg mt-3">{esc((item as Contact).message)}</p>
                  )}
                  {item._type === "survey" && (
                    <div className="mt-3 text-xs text-n-500 space-y-1 bg-n-50 p-3 rounded-lg">
                  ...
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => createPaymentFor(item.name, item.phone)} className="text-xs px-3 py-1.5 rounded-lg border border-n-300 text-n-600 hover:bg-n-50">결제 링크 생성</button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 z-[200] flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl p-8 max-w-md w-full" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold text-n-900 mb-4">결제 링크 생성 완료</h3>
            <p className="text-sm text-n-500 mb-3">아래 링크를 고객에게 보내주세요:</p>
            <div className="bg-n-50 p-3 rounded-lg text-xs text-n-600 break-all mb-4">{modalLink}</div>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 rounded-lg bg-n-100 text-n-600 text-sm">닫기</button>
              <button onClick={() => { navigator.clipboard.writeText(modalLink).then(() => alert("복사됨!")); }} className="px-4 py-2 rounded-lg bg-n-900 text-white text-sm font-medium">링크 복사</button>
            </div>
          </div>
        </div>
      )}
