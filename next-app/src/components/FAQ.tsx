  ...
  return (
    <div className={`border-b border-n-100 ${index === 0 ? "border-t" : ""}`}>
      <button onClick={() => setOpen(!open)}
        ...
        <span className="text-sm md:text-[15px] font-medium text-n-700 group-hover:text-n-900 transition-colors leading-relaxed">
          {q}
        </span>
        <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${open ? "bg-n-900" : "border border-n-200"}`}>
      ...
      <div className="mx-auto max-w-6xl px-6">
        <FadeIn>
          <div className="text-center mb-14">
            <p className="text-[11px] uppercase tracking-[0.2em] text-n-400 font-medium mb-6">FAQ</p>
        ...
        <FadeIn>
          <div className="max-w-2xl mx-auto mt-10 text-center">
            <p className="text-sm text-n-400 mb-2">더 궁금한 점이 있으신가요?</p>
            <a href="#contact" className="text-sm font-medium text-n-600 hover:text-n-900 transition-colors inline-flex items-center gap-1.5">
              편하게 상담 신청해주세요
